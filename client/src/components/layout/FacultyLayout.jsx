...
const FACULTY_NAV = [
  ...
  { to: "/faculty/interns", label: "My Interns", icon: Users },
  { to: "/faculty/letters", label: "Internship Letters", icon: FileCheck },
  { to: "/faculty/reviews", label: "Performance Reviews", icon: MessageSquarePlus },
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      <aside className="fixed inset-y-0 left-0 z-40 w-56 flex flex-col border-r border-gray-200 bg-white px-3 py-5">
        ...
        {/* Faculty badge */}
        <div className="mx-2 mb-4 flex items-center gap-2.5 rounded-xl bg-amber-50 border border-amber-100 p-2.5">
          <div className="w-8 h-8 rounded-full bg-amber-600 flex items-center justify-center text-white text-xs font-extrabold shrink-0">
            {user.name?.charAt(0) || "F"}
          </div>
          <div className="min-w-0">
            <p className="text-xs font-bold text-gray-900 truncate">{user.name}</p>
            <p className="text-[10px] text-amber-600 font-semibold">Faculty</p>
          </div>
        </div>

        {/* Navigation */}
        <nav className="flex flex-col gap-1.5 flex-1" aria-label="Faculty navigation">
          {FACULTY_NAV.map(({ to, label, icon: Icon, end }) => {
            const isActive = end
      ...
      <div className="ml-56 flex flex-1 flex-col min-h-screen">
        ...
        <header className="sticky top-0 z-30 flex h-14 items-center justify-between gap-4 border-b border-gray-200 bg-white px-6">
          ...
          {/* Search */}
          <div className="flex flex-1 max-w-sm items-center gap-2 rounded-xl border border-gray-200 bg-gray-50 px-3.5 py-1.5 transition-colors focus-within:border-amber-400 focus-within:ring-2 focus-within:ring-amber-100">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2.5 pl-3 border-l border-gray-200">
              <span className="text-[11px] font-extrabold px-3 py-1 rounded-full bg-amber-50 text-amber-700 border border-amber-200">
                Faculty
              </span>
              <button
                onClick={handleLogout}
