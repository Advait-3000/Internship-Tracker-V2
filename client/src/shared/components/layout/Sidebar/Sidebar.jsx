...
const getIcon = (label) => {
  ...
  if (l.includes("dashboard") || l.includes("report") || l.includes("log") || l.includes("audit"))
    return DashboardIcon;

  if (l.includes("student") || l.includes("user management") || l.includes("mentee") || l.includes("attendance"))
    return StudentsIcon;

  if (l.includes("internship") || l.includes("application") || l.includes("company") || l.includes("companies"))
    return CompaniesIcon;

  if (l.includes("faculty") || l.includes("evaluation"))
    return FacultyIcon;

  if (l.includes("profile") || l.includes("setting"))
...
const Sidebar = ({ links = [] }) => {
  return (
    <aside className="w-[220px] h-screen bg-white border-r border-gray-200 min-h-[calc(100vh-65px)] p-4 flex flex-col">
      {/* AUM Logo Header */}
      <div className="flex items-center gap-6 px-5 pb-5 border-b border-gray-100 mb-4 h-[12.5vh]">
        <img src={AUMLogo} alt="AUM Logo" className="w-[60px] h-[70px] object-contain" />
