  ...
  return (
    <div className="w-full">
      <div className="flex justify-between text-[11px] font-semibold mb-1">
        ...
        <span
          className={`font-bold ${pct >= 90 ? "text-red-600" : pct >= 60 ? "text-amber-600" : "text-emerald-600"}`}
        >
          {current} / {quota}
        </span>
      </div>
...
const AdminManagement = () => {
  ...
  const assignedFacs = new Set(adminList.flatMap((a) => a.managedFacultyIds))
    .size;
  ...
  const openEdit = (admin) => {
    ...
    setForm({
      ...
      role: admin.role,
      status: admin.status,
      facultyQuota: admin.facultyQuota,
      password: "",
    });
    setShowPassword(false);
    setIsFormModalOpen(true);
  ...
  const handleAssign = (adminId, facId) =>
    dispatch(assignFacultyToAdmin({ adminId, facultyId: facId }));
  const handleUnassign = (adminId, facId) =>
    dispatch(removeFacultyFromAdmin({ adminId, facultyId: facId }));
    ...
    <div className="space-y-6 pb-12">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-5">
          <div>
            ...
            <p className="text-sm text-gray-300 mt-1 max-w-lg">
              Create and manage admin accounts. Each admin oversees a set of
              faculty members who in turn guide students.
            </p>
          </div>
          <button
            onClick={openAdd}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {filtered.map((admin, idx) => {
          ...
          const managedFaculties = facultyList.filter((f) =>
            admin.managedFacultyIds.includes(f.id),
          );
            ...
            >
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3.5">
                  <div
                    className={`w-12 h-12 rounded-xl bg-gradient-to-br ${gradient} text-white flex items-center justify-center text-sm font-extrabold shrink-0 group-hover:scale-105 transition-transform shadow-md`}
                  >
                ...
                <span
                  className={`text-[11px] font-bold px-2.5 py-1 rounded-full ${statusPill(admin.status)}`}
                >
                  {admin.status}
                </span>
              </div>
                ...
                {managedFaculties.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-2.5">
                    {managedFaculties.slice(0, 3).map((f) => (
                      <span
                        key={f.id}
                        className="text-[10px] font-semibold px-2 py-0.5 rounded-md bg-violet-50 text-violet-700 border border-violet-100"
                      >
                        {f.name}
                      </span>
                    ))}
          ...
          >
            ...
            <p className="text-xs text-gray-500 mb-5">
              {editingId
                ...
                : "Create a new admin who will oversee faculty members."}
            </p>
              ...
              <div>
                <label className="block text-xs font-bold text-gray-700 mb-1.5">
                  ...
                  <span className="text-gray-400 font-normal">
                    (max faculties this admin can oversee)
                  </span>
                </label>
                <input
                  type="number"
                  min={1}
                  max={20}
                  value={form.facultyQuota}
                  onChange={(e) =>
                    setForm((f) => ({
                      ...f,
                      facultyQuota: Number(e.target.value),
                    }))
                  }
                  className="w-full px-4 py-2.5 text-xs bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-violet-500 focus:bg-white"
                />
      ...
      {deleteConfirmId && (
        ...
        >
          <div
            className="bg-white rounded-[28px] shadow-2xl max-w-sm w-full border border-gray-100 p-6 sm:p-8 relative"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <p className="text-xs text-gray-500 mb-6">
              ...
              <span className="font-bold text-gray-800">
                {adminList.find((a) => a.id === deleteConfirmId)?.name}
              </span>
              ? All faculty assignments will be lost.
            </p>
      ...
      {detailAdmin &&
        (() => {
          ...
          const unassignedFac = facultyList.filter(
            (f) => !assignedIds.has(f.id),
          );
          const assignedFac = facultyList.filter((f) => assignedIds.has(f.id));
          const gradient =
            AVATAR_GRADIENTS[
              adminList.indexOf(admin) % AVATAR_GRADIENTS.length
            ];
          return (
            <div
              className="fixed inset-0 z-50 flex items-end sm:items-center justify-end bg-black/40 backdrop-blur-sm p-0 sm:p-4 animate-in fade-in duration-200"
              onClick={() => setDetailAdmin(null)}
            >
              <div
                className="bg-white rounded-t-[28px] sm:rounded-[28px] shadow-2xl w-full sm:w-[480px] max-h-[90vh] overflow-y-auto border border-gray-100"
                onClick={(e) => e.stopPropagation()}
              >
                {/* Drawer header */}
                <div className={`bg-gradient-to-r ${gradient} p-6 text-white`}>
                  <div className="flex items-start justify-between">
                    ...
                    <button
                      onClick={() => setDetailAdmin(null)}
                      className="p-2 rounded-xl bg-white/20 hover:bg-white/30 cursor-pointer"
                    >
                      <X className="w-5 h-5" />
                    </button>
                  </div>
                  <div className="mt-4">
                    <FacultyProgressBar
                      current={assignedFac.length}
                      quota={admin.facultyQuota}
                    />
                  </div>
                </div>
                          ...
                          >
                            <div>
                              ...
                              <p className="text-[11px] text-gray-500">
                                {f.department} · {f.assignedStudents.length}{" "}
                                students
                              </p>
                            </div>
                            <button
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          {unassignedFac.map((f) => (
                            <div
                              key={f.id}
                              className="flex items-center justify-between p-3 rounded-xl bg-gray-50 border border-dashed border-gray-200"
                            >
                              <div>
                                <p className="text-xs font-bold text-gray-900">
                                  {f.name}
                  ...
                  {assignedFac.length >= admin.facultyQuota && (
                    <div className="flex items-center gap-2 p-3 rounded-xl bg-amber-50 border border-amber-200">
                      ...
                      <p className="text-xs text-amber-700 font-semibold">
                        Faculty quota reached. Increase quota or unassign a
                        faculty first.
                      </p>
                    </div>
                  )}
